...
- 测试采用精简、风险驱动的策略，选择适量且具有代表性的测试用例，不追求测试数量或穷举覆盖。
- 优先覆盖核心成功路径、关键业务边界、权限校验、安全规则和高风险并发状态转换。
- 禁止为相似输入排列组合、重复 Mockito 调用验证或低风险实现细节编写大量重复测试。
- 开发过程中优先运行当前模块或受影响范围的定向测试；完整功能完成后再统一运行一次全量测试和启动验证。
- 在保证核心功能与高风险场景得到验证的前提下，减少测试编写、执行时间和 token 消耗。
