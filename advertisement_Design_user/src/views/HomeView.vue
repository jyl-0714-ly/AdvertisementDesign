    ...
    <main>
      ...
      <section id="portfolio" class="portfolio-section">
        <div class="section-intro">
          ...
          <p>从品牌系统、线上传播到线下物料，以代表性项目呈现我们的设计方法。</p>
        </div>

        <div class="portfolio-category-tabs" role="tablist" aria-label="案例分类">
        ...
        <div class="portfolio-filter-bar">
          <el-input v-model="filters.keyword" placeholder="搜索项目、行业或设计风格" :prefix-icon="Search" clearable />
        ...
        <div v-loading="loading" class="portfolio-grid">
          <article v-for="item in cases" :key="item.id" class="portfolio-card" @click="openCase(item.id)">
            <div class="portfolio-cover" :style="coverStyle(item)">
              <span>{{ portfolioCategoryLabel(item.category) }} · {{ item.industry }}</span>
      ...
      <section id="process" class="public-process">
        ...
        <div class="collaboration-modes">
          <article class="collaboration-mode online">
            <header><el-icon><Monitor /></el-icon><div><span>ONLINE</span><h3>线上合作</h3></div></header>
            <ol><li><b>01</b><span>在线提交需求</span></li><li><b>02</b><span>远程方案沟通</span></li><li><b>03</b><span>线上审稿确认</span></li><li><b>04</b><span>数字文件交付</span></li></ol>
          </article>
          <article class="collaboration-mode offline">
            <header><el-icon><Location /></el-icon><div><span>OFFLINE</span><h3>线下合作</h3></div></header>
            <ol><li><b>01</b><span>到店或上门沟通</span></li><li><b>02</b><span>现场调研勘察</span></li><li><b>03</b><span>实物打样确认</span></li><li><b>04</b><span>成品制作交付</span></li></ol>
          </article>
        </div>
      </section>
    </main>
...
import { ArrowRight, Location, LocationInformation, Message, Monitor, Phone, Search, Service } from '@element-plus/icons-vue'
...
const industries = computed(() => [...new Set(featuredCases.value.map((item) => item.industry))])
const styles = computed(() => [...new Set(featuredCases.value.map((item) => item.style))])
const cases = computed(() => {
  const keyword = filters.keyword.trim().toLowerCase()
  return featuredCases.value.filter((item) =>
    (!filters.category || item.category === filters.category)
    && (!filters.industry || item.industry === filters.industry)
    && (!filters.style || item.style === filters.style)
    && (!keyword || `${item.title}${item.description}${item.industry}${item.style}${item.serviceType}`.toLowerCase().includes(keyword))
  )
})
...
async function loadFeatured() {
  loading.value = true
  try {
    const page = await listPortfolioCases({ featured: true, page: 1, size: 6 })
    featuredCases.value = page.records
  } catch (error) {
...
function reset() {
  Object.assign(filters, { keyword: '', category: '', industry: '', style: '' })
}
...
function openCase(id: number) {
  rememberPortfolioPosition(route.fullPath)
...
.portfolio-category-tabs { margin-bottom: 14px; display: flex; gap: 28px; overflow-x: auto; border-bottom: 1px solid #dfe6ee; }.portfolio-category-tabs button { min-width: max-content; padding: 0 0 12px; border: 0; border-bottom: 2px solid transparent; background: transparent; color: #758296; cursor: pointer; font-size: 14px; transition: color .18s ease, border-color .18s ease; }.portfolio-category-tabs button:hover { color: #172033; }.portfolio-category-tabs button.active { border-bottom-color: #172033; color: #172033; font-weight: 700; }
.portfolio-filter-bar { display: grid; grid-template-columns: 1.6fr 1fr 1fr auto; gap: 10px; padding: 12px; margin-bottom: 27px; border: 1px solid #e7edf5; background: #fff; border-radius: 8px; }.portfolio-filter-bar .el-input__wrapper, .portfolio-filter-bar .el-select__wrapper { box-shadow: none; background: #f6f8fb; min-height: 39px; }.filter-reset { padding: 0 11px; color: #47617e; }
...
.public-process { padding-top: 34px; padding-bottom: 80px; }.section-intro.compact { margin-bottom: 23px; }.collaboration-modes { display: grid; grid-template-columns: 1fr 1fr; gap: 18px; }.collaboration-mode { padding: 28px; border: 1px solid #dfe5ec; border-radius: 8px; background: #fff; }.collaboration-mode header { display: flex; align-items: center; gap: 14px; padding-bottom: 22px; border-bottom: 1px solid #e8edf2; }.collaboration-mode header > .el-icon { width: 42px; height: 42px; border-radius: 8px; display: grid; place-items: center; background: #edf4fc; color: #1367d1; font-size: 21px; }.collaboration-mode.offline header > .el-icon { background: #fff3e8; color: #b55d18; }.collaboration-mode header span { color: #8793a3; font-size: 10px; font-weight: 700; letter-spacing: 0; }.collaboration-mode h3 { margin: 4px 0 0; font-size: 20px; }.collaboration-mode ol { margin: 0; padding: 4px 0 0; list-style: none; }.collaboration-mode li { min-height: 56px; display: grid; grid-template-columns: 38px 1fr; align-items: center; border-bottom: 1px solid #edf1f4; }.collaboration-mode li:last-child { border-bottom: 0; }.collaboration-mode li b { color: #1367d1; font-size: 11px; }.collaboration-mode.offline li b { color: #b55d18; }.collaboration-mode li span { color: #344256; font-size: 14px; font-weight: 650; }
...
@media (max-width: 820px) { .public-header { padding: 0 18px; }.public-nav { gap: 12px; }.public-nav a { display: none; }.public-intro { grid-template-columns: 1fr; padding-top: 52px; gap: 35px; }.public-intro h1 { font-size: 38px; }.public-art { height: 270px; }.art-card-main { left: 32px; width: 200px; height: 220px; }.art-card-side { right: 25px; bottom: 24px; }.portfolio-grid { grid-template-columns: 1fr; }.portfolio-filter-bar { grid-template-columns: 1fr 1fr; }.portfolio-filter-bar > :first-child { grid-column: 1 / -1; }.section-intro { align-items: flex-start; flex-direction: column; }.collaboration-modes { grid-template-columns: 1fr; } }
@media (max-width: 520px) { .portfolio-filter-bar { grid-template-columns: 1fr; }.portfolio-filter-bar > :first-child { grid-column: auto; }.portfolio-filter-bar .filter-reset { min-height: 36px; }.collaboration-mode { padding: 22px; } }
</style>
