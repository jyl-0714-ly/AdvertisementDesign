<template>
  <div class="workspace-portfolio" v-loading="loading">
    <header class="workspace-portfolio-head">
      <div><span class="workspace-eyebrow">PORTFOLIO</span><h1>作品集</h1><p>浏览公开案例，为你的项目找到更接近的视觉方向。</p></div>
    ...
    <div class="workspace-filter-bar">
      ...
      <el-select v-model="filters.serviceType" clearable placeholder="服务类型"><el-option v-for="item in serviceTypes" :key="item" :label="item" :value="item" /></el-select>
      <button type="button" @click="reset">重置</button>
    </div>

    <div class="workspace-case-grid">
      <article v-for="item in filteredCases" :key="item.id" class="workspace-case-card" @click="router.push(`/cases/${item.id}`)">
        <div class="workspace-case-cover" :style="coverStyle(item)"><span>{{ item.industry }}</span><b>{{ item.serviceType }}</b><i>{{ String(item.id).padStart(2, '0') }}</i></div>
...
const filteredCases = computed(() => {
  ...
  return cases.value.filter((item) => (!filters.industry || item.industry === filters.industry) && (!filters.style || item.style === filters.style) && (!filters.serviceType || item.serviceType === filters.serviceType) && (!keyword || `${item.title}${item.description}${item.industry}${item.style}${item.serviceType}`.toLowerCase().includes(keyword)))
})
