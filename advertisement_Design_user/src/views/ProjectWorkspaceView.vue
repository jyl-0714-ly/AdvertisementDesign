<template>
  <div class="project-workspace-view">
    ...
    <template v-else-if="state?.projection">
      <ProjectConversationPane :projection="state.projection" :state="state" @load-older="workspace.loadMessages(projectId)" @files="workspace.addFiles(projectId,$event)" @retry="workspace.uploadAttachment(projectId,$event)" @remove="workspace.removeAttachment(projectId,$event)" @correct="workspace.beginCorrection(projectId,$event)" @cancel-correction="workspace.cancelCorrection(projectId)" @send="workspace.send(projectId)"/>
      <ProjectInspectorPane :projection="state.projection" @history="workspace.loadStageHistory(projectId,$event)" @action="handleAction" @rename="renameProject" @restore-auto="restoreAutomaticNaming"/>
    </template>
  </div>
</template>
...
async function renameProject(name:string){if(await workspace.renameProject(projectId.value,name))ElMessage.success('项目名称已更新。')}
async function restoreAutomaticNaming(){if(await workspace.restoreAutomaticNaming(projectId.value))ElMessage.success('已恢复自动命名。')}
